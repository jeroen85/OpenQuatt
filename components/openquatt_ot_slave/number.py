...
OpenQuattOTSlaveNumber = generate.openquatt_ot_slave_ns.class_("OpenQuattOTSlaveNumber", number.Number, cg.Component, input.OpenQuattOTSlaveInput)

async def new_openquatt_ot_slavenumber(config: Dict[str, Any]) -> cg.Pvariable:
...
def get_entity_validation_schema(entity: schema.InputSchema) -> cv.Schema:
    return number.NUMBER_SCHEMA \
        .extend({
            cv.GenerateID(): cv.declare_id(OpenQuattOTSlaveNumber),
...
async def to_code(config: Dict[str, Any]) -> None:
    keys = await generate.component_to_code(
        COMPONENT_TYPE,
        schema.INPUTS,
        OpenQuattOTSlaveNumber, 
        generate.create_only_conf(new_openquatt_ot_slavenumber), 
        config
    )
