...
Kenmerken: Flowmeter in de buitenunit ge\xEFntegreerd. Onder CV-ketel enkel een kleine clip-on temperatuursensor.`}},t=n.entities.hpGeneration||{},o=String(T("hpGeneration")||""),r=dn(t),a=n.loadingEntities||n.busyAction==="save-hpGeneration";return`
    ...
    `}function Pb(e){const t=Math.max(1,Number(e||0)/1e3),o=Math.pow(10,Math.floor(Math.log10(t))),r=t/o;return(r<=1.5?1.5:r<=3?3:r<=6?6:10)*o*1e3}function Hb(e){const t=Number(e);return Number.isFinite(t)?t>=999500?`${Number((t/1e6).toFixed(1))}`:`${Number((t/1e3).toFixed(1))}`:""}function Nb(e){return e>=999500?"MWh":"kWh"}function Rb(e){const c=Math.max(1e3,...e.map(wb)),p=Pb(c),f=e.length?1218/e.length:1218,m=Math.max(3.2,Math.min(22,f*.36));return{width:1280,height:260,left:44,right:18,top:26,bottom:38,plotWidth:1218,plotHeight:196,axisMax:p,barSlot:f,barWidth:m,yOf:y=>26+(1-Math.min(1,Math.max(0,Number(y||0)/p)))*196}}function Lb(e){const t=Vt(e.heatpumpHeatOutputWh,e.heatingInputWh),o=Vt(e.heatpumpCoolingOutputWh,e.coolingInputWh),r=Vt(Gc(e),e.electricalInputWh);return[e.tooltipLabel||e.label||qt(e.dateKey),`Elektrisch: ${Te(e.electricalInputWh,1)}`,`Warmtepomp warmte: ${Te(e.heatpumpHeatOutputWh,1)}`,`Warmtepomp koeling: ${Te(e.heatpumpCoolingOutputWh,1)}`,`Cv-ketel: ${Te(e.boilerHeatOutputWh,1)}`,`COP verwarmen: ${t}`,`EER koelen: ${o}`,`Output / elektrisch: ${r}`].join(`
